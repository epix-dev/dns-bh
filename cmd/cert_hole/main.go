...
func updateItems(db *sql.DB, items []domain) []string {
	...
	inserSQL := `
	INSERT INTO cert_hole ( remote_id, domain, insert_time, created_at )
		VALUES ($1, $2, $3, CURRENT_TIMESTAMP)
	`

	updateSQL := "UPDATE cert_hole SET updated_at = CURRENT_TIMESTAMP WHERE remote_id = $1"
	...
	for _, item := range items {
		...
		updateRes, err := updateStmt.Exec(item.RegisterPositionID)
		lib.CheckError(err)
...
func deleteItems(db *sql.DB, items []domain) []string {
	var result []string

	updateSQL := `
		UPDATE cert_hole
			SET updated_at = CURRENT_TIMESTAMP, deleted_at = CURRENT_TIMESTAMP
		WHERE remote_id = $1 AND deleted_at IS NULL
		`
	...
	for _, item := range items {
		if !item.DeleteTime.Valid { // skip domains without DeleteTime
			continue
		}
		...
		if affect, _ := updateRes.RowsAffected(); affect == 1 {
			log.Printf("deleted %d:%s", item.RegisterPositionID, item.Domain)
			result = append(result, item.Domain)
		}

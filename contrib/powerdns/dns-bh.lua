...
function preresolve(dq)
   if hazard:check(dq.qname) then
       if dq.qtype == pdns.A then
           log_entry = string.format("hazard domain query type %s from %s, (REWRITE): %s", dq.qtype, dq.remoteaddr:toString(), dq.qname:toString() )
           pdnslog(log_entry, pdns.loglevels.Info)
           dq:addAnswer(pdns.A, hazard_response_a)
           return true;
        else
           log_entry = string.format("hazard domain query type %s from %s, (NODATA): %s", dq.qtype, dq.remoteaddr:toString(), dq.qname:toString() )
           pdnslog(log_entry, pdns.loglevels.Info)
   ...
   if malware:check(dq.qname) then
       log_entry = string.format("malware domain query type %s from %s, (NXDOMAIN): %s", dq.qtype, dq.remoteaddr:toString(), dq.qname:toString() )
       pdnslog(log_entry, pdns.loglevels.Info)
